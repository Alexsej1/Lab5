    ...
    match role:
        case Role.admin:
            return [
                ...
                TokenScope.read_financial,
                TokenScope.write_financial,
            ]
        case Role.user:
...
def require_all_scopes(scopes: List[TokenScope]):
    async def scope_checker(
        ...
        if not all(scope.value in user_scopes for scope in scopes):
            raise HTTPException(
                status_code=403,
                detail=f"Недостаточно прав. Требуемые скоупы: {', '.join(s.value for s in scopes)}",
            )
...
async def register(user: UserModel, db=Depends(get_db)):
    db_user = await get_user_by_username(user.username, db)
    if db_user:
        raise HTTPException(status_code=400, detail="ЛОигн уже занят")
...
async def read_user_route(
    ...
    return {"message": f"Добро пожаловать, {current_user.username}.Вы пользователь."}
...
async def get_financial_data(
    current_user: User = Depends(require_all_scopes([TokenScope.read_financial])),
):
    return {
        "message": f"Добро пожаловать, {current_user.username}. Это конфиденциальные финансовые данные.",
        "data": [
            {"year": 2024, "usd": 150},
            {"year": 2025, "usd": 180000},
        ],
    }


@router.post("/financial_data/update")
async def update_financial_data(
    current_user: User = Depends(require_all_scopes([TokenScope.write_financial])),
):
    return {
        "message": f"Добро пожаловать, {current_user.username}. Финансовые данные успешно обновлены.",
    }